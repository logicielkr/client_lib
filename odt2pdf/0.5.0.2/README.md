...
모바일과 같이
화면의 width 가 A4 나 Letter 보다 작은 경우
scale 을 변경해서 화면에 끼워 맞추는 기능은 아쉬움과 함께 구현하였지만,
border 가 사라지는 현상이 있다.
...
2. 의미없는 colspan 을 사용한 경우 ```<td>``` 의 width 오류
	...
	.odt 파일에는 내부적으로 colspan 이 있는 경우가 있는데,
	이런 경우 width 가 .odt 파일에 style 로 정의한 것과 다르게 표시된다.
	IE 는 이런 논리적 오류가 있는 경우 ```width:auto;``` 로 변경하여 처리하는 것으로 생각된다.
	이 문제는 ```table-layout: fixed;``` 를 추가하는 방법으로 해결할 수 있지만, 처리를 보류하였다.
