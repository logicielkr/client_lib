...
GrahaFormula.Func.prototype.valueOf = function() {
	...
	for(var i = 0; i < this.element.length; i++) {
		if(this.element[i] == "," || this.element[i] == ";") {
			if(last == (i - 1)) {
...
GrahaFormula.Func.prototype.last = function(expr) {
	if(arguments.length == 0) {
		...
		if(this.element.length > 0) {
			this.element[this.element.length - 1] = expr;
		} else {
			this.append(expr);
		}
	}
...
GrahaFormula.Func.udf.if = function(data) {
	if(data.length > 0) {
//		var value = GrahaFormula.Func.udf.bool(data[0]);
		var value = data[0].valueOf();
		if(value) {
			if(data.length > 1) {
				return data[1].valueOf();
			} else {
				return true;
			}
		} else {
...
GrahaFormula.Func.udf.bool = function() {
	var left;
	var oper;
	var right;
	if(arguments.length == 1) {
		if(arguments[0].length) {
			if(typeof(arguments[0][0]) == "boolean" && typeof(arguments[0][2]) == "boolean") {
				left = arguments[0][0].valueOf();
				right = arguments[0][2].valueOf();
			} else if(
				GrahaFormula.Func.udf._isNumber(arguments[0][0]) &&
				GrahaFormula.Func.udf._isNumber(arguments[0][2])
			) {
				left = GrahaFormula.Func.udf.number(arguments[0][0]);
				right = GrahaFormula.Func.udf.number(arguments[0][2]);
			} else {
				left = GrahaFormula.Func.udf.string(arguments[0][0]);
				right = GrahaFormula.Func.udf.string(arguments[0][2]);
			}
			oper = arguments[0][1];
		} else if(arguments[0].element) {
			if(typeof(arguments[0].element[0]) == "boolean" && typeof(arguments[0].element[2]) == "boolean") {
				left = arguments[0].element[0].valueOf();
				right = arguments[0].element[2].valueOf();
			} else if(
				GrahaFormula.Func.udf._isNumber(arguments[0].element[0]) &&
				GrahaFormula.Func.udf._isNumber(arguments[0].element[2])
			) {
				left = GrahaFormula.Func.udf.number(arguments[0].element[0]);
				right = GrahaFormula.Func.udf.number(arguments[0].element[2]);
			} else {
				left = GrahaFormula.Func.udf.string(arguments[0].element[0]);
				right = GrahaFormula.Func.udf.string(arguments[0].element[2]);
			}
			oper = arguments[0].element[1];
		}
	} else if(arguments.length == 3) {
		if(typeof(arguments[0]) == "boolean" && typeof(arguments[2]) == "boolean") {
			left = arguments[0].valueOf();
			right = arguments[2].valueOf();
		} else if(
			GrahaFormula.Func.udf._isNumber(arguments[0]) &&
			GrahaFormula.Func.udf._isNumber(arguments[2])
		) {
			left = GrahaFormula.Func.udf.number(arguments[0]);
			right = GrahaFormula.Func.udf.number(arguments[2]);
		} else {
			left = GrahaFormula.Func.udf.string(arguments[0]);
			right = GrahaFormula.Func.udf.string(arguments[2]);
		}
		oper = arguments[1];
	} else {
		return NaN;
	}
	if(oper instanceof GrahaFormula.Oper) {
		if(oper == ">") {
			return (left > right);
		} else if(oper == ">=") {
			return (left >= right);
...
GrahaFormula.Expr.prototype.last = function(expr) {
	if(arguments.length == 0) {
		if(this.element.length > 0) {
			return this.element[this.element.length - 1];
		} else {
...
GrahaFormula.Expr.prototype.valueOf = function() {
	...
	for(var z = 0; z < clone.length; z++) {
		var a = clone[z];
		if(a instanceof GrahaFormula.Oper && (a == ">" || a == ">=" || a == "<" || a == "<=" || a == "=" || a == "==")) {
			var prevIndex = this.prevIndex(z, clone);
			var nextIndex = this.nextIndex(z, clone);
			if(isNaN(prevIndex) || isNaN(nextIndex)) {
				throw new Error("unknown syntax");
				...
				clone[prevIndex] = null;
				clone[z] = null;
				clone[nextIndex] = new GrahaFormula.Const(GrahaFormula.Func.udf.bool(prev, a, next), "boolean");
				z = nextIndex;
			}
		}
	}
		...
		if(x == "\"" && !isQ && qT == null) {
			...
			if(x == "(") {
				...
				if(t.trim() != "") {
					...
					if(
						x == "=" &&
						result[current].last() != null &&
						result[current].last() instanceof GrahaFormula.Oper &&
						(
							result[current].last() == "<" ||
							result[current].last() == ">" ||
							result[current].last() == "="
						...
						result[current].last(new GrahaFormula.Oper(result[current].last() + x));
					} else if(
						x == "&" &&
						result[current].last() != null &&
						result[current].last() instanceof GrahaFormula.Oper &&
