...
GrahaReporter.create = function(dataUrl, entries, fileName, mimeType) {
	...
	$.ajax({
		...
		success: function(data, textStatus, jqXHR) {
			GrahaReporter.dataUrl = dataUrl;
			GrahaReporter.fileName = fileName;
			GrahaReporter.entries = entries;
			if(mimeType) {
				GrahaReporter.mimeType = mimeType;
			} else {
				GrahaReporter.mimeType = null;
			}
			for(var i = 0; i < entries.length; i++) {
				GrahaReporter.process(data, entries[i], i);
			}
		},
