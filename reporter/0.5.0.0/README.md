...
2. 한/글 만 지원하는 특수문자
	...
	한/글이 아닌 다른 프로그램에서는 글자가 깨진 것처럼 보일 수 있다는 것에 주의한다.
	
	> 다른 프로그램에서 글자가 깨지는 것이 싫다면, 동그라미 안에 있는 "인" 대신에 (인) 이나 ㊞ 을 사용하는 것을 검토한다.
	
	다국어를 불완전하게 지원하는 editor 를 사용한다면,
	저장하면서 특수문자가 깨질 수 있다(이 경우는 필자가 사용하는 jEdit 를 추천한다).
	
	> xslt 파일의 가독성을 위하여 ENTITY 로 정의하여 사용하는 것을 검토한다.
