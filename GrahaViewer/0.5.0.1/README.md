...
https://github.com/logicielkr/client_lib/tree/master/odt2pdf/0.5.0.1
...
- styles.xml 에 각주(footnote) 가 정의되지 않았다면, content.xml 에 각주(footnote) 가 있어도 무시된다.
- list 는 무시된다.
- 링크 (```<text:a>```) 는 무시된다.
- styles.xml 과 content.xml 에 오류가 있는 경우 [Apache OpenOffice](https://www.openoffice.org/) 나 [LibreOffice](https://www.libreoffice.org/) 에서는 이를 적당히 처리하는 데 GrahaOdt2PdfConverter 는 적당한 처리가 생략되었다.
- Tab (```<text:tab>```) 은 무시된다.
